...
let ``Creacion de empleado con datos validos y ambos apellidos se ejecuta correctamente`` () =
    let ci = match CI.crear "1234567" (Some "1A") (Some LP) with Ok c -> c | Error e -> failwithf "%A" e
    
    match Empleado.crearDeDatos "Juan Carlos" (Some "Pérez") (Some "Martínez") ci (Some "+591 71234567") (Some "juan.perez@empresa.com") with
    | Ok emp ->
        ...
        Assert.Equal(Some "1A", emp.CI.Complemento)
        Assert.Equal(Some LP, emp.CI.Extension)
        Assert.Equal("1234567-1A LP", CI.formatear emp.CI)
...
[<Fact>]
let ``Composicion de Persona en Empleado delega correctamente la identidad personal`` () =
    let ci = match CI.crear "6543210" None (Some CB) with Ok c -> c | Error e -> failwithf "%A" e
    let resPersona = Persona.crear "Gabriel" None (Some "Torrico") ci None None
    match resPersona with
    | Error e -> failwithf "Falló creación de persona: %A" e
    | Ok persona ->
        let emp = Empleado.crear persona
        Assert.Equal("Gabriel", emp.DatosPersonales.Nombres)
...
let ``Creacion de empleado con solo apellido materno (caso docente) es permitida y formatea correctamente`` () =
    let ci = match CI.crear "9876543" None None with Ok c -> c | Error e -> failwithf "%A" e

    match Empleado.crearDeDatos "Gabriel" None (Some "Torrico") ci None None with
    | Ok emp ->
        Assert.Equal("Gabriel", emp.Nombres)
...
let ``Creacion de empleado con solo apellido paterno es permitida y formatea correctamente`` () =
    let ci = match CI.crear "4567890" None None with Ok c -> c | Error e -> failwithf "%A" e

    match Empleado.crearDeDatos "María Elena" (Some "Rivas") None ci None None with
    | Ok emp ->
...
let ``Creacion de empleado sin ningun apellido retorna error ValorRequerido`` () =
    let ci = match CI.crear "1122334" None None with Ok c -> c | Error e -> failwithf "%A" e

    match Empleado.crearDeDatos "Juan Daniel" None None ci None None with
...
let ``Creacion de empleado con numeros en nombres o apellidos retorna error SimbolosNoPermitidos`` () =
    let ci = match CI.crear "8765432" None None with Ok c -> c | Error e -> failwithf "%A" e

    // Número en nombre
    match Empleado.crearDeDatos "Carlos123" (Some "Pérez") None ci None None with
    ...
    match Empleado.crearDeDatos "Carlos" (Some "Perez99") None ci None None with
    | Error (SimbolosNoPermitidos msg) -> Assert.Contains("números", msg)
    | res -> failwithf "Debería haber fallado por paterno con números, obtuvo: %A" res

    // Número en apellido materno
    match Empleado.crearDeDatos "Carlos" None (Some "Gomez2") ci None None with
    | Error (SimbolosNoPermitidos msg) -> Assert.Contains("números", msg)
...
let ``Creacion de empleado con letras en el telefono retorna error LetrasNoPermitidas`` () =
    ...
    match Empleado.crearDeDatos "Carlos" (Some "Mendoza") None ci (Some "71234567abc") None with
...
let ``Inactivar y activar empleado actualiza su estado inmutablemente`` () =
    let ci = match CI.crear "5544332" None None with Ok c -> c | Error e -> failwithf "%A" e
    let emp = match Empleado.crearDeDatos "Ana María" (Some "Ríos") None ci None None with Ok e -> e | Error err -> failwithf "%A" err
...
let ``Actualizar empleado modifica sus datos preservando identidad y estado`` () =
    let ci = match CI.crear "1234567" None None with Ok c -> c | Error e -> failwithf "%A" e
    let ciNueva = match CI.crear "7654321" (Some "CB") (Some CB) with Ok c -> c | Error e -> failwithf "%A" e
    let original = match Empleado.crearDeDatos "Pedro" (Some "Ramos") None ci None None with Ok e -> e | Error err -> failwithf "%A" err
